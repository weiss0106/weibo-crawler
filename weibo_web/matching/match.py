...
class AnalyzeSensitivity():
    def __init__(self, sensitive_word, mysql_config):
    ...
    def search(self):
        ...
        # 使用cursor()方法创建一个游标对象
        cursor = db.cursor()
        sql1 = "SELECT text,uid FROM comment"
...
def generate_cloud(key, result):
    ...
    wc = WordCloud(background_color="white", mask=background_img, stopwords=STOPWORDS,
                   font_path='../weibo_crawler/SourceHanSans-Bold.ttf').generate(res_text)
...
def main(param, n):
    ...
    if n == 1:
        ...
        s = AnalyzeSensitivity(param, mysql_config)
        s.search()
