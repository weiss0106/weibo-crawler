...
class CsvPipeline(object):
    def process_item(self, item, spider):
        base_dir = 'weibo_results/topic' + os.sep + item['keyword']
...
class MyImagesPipeline(ImagesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        ...
        base_dir = 'weibo_results/topic' + os.sep + item['keyword'] + os.sep + 'images'
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
...
class MyVideoPipeline(FilesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        item = request.meta['item']
        base_dir = 'weibo_results/topic' + os.sep + item['keyword'] + os.sep + 'videos'
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
...
class MysqlPipeline(object):
    ...
    def open_spider(self, spider):
        try:
            ...
            mysql_config = {
                ...
                'user': settings.get('MYSQL_USER', 'lab'),
    ...
    def process_item(self, item, spider):
        ...
        sql = """INSERT INTO {table}({keys}) VALUES ({values}) ON
                     DUPLICATE KEY UPDATE""".format(table='topic',
                                                    keys=keys,
                                                    values=values)
